...
# API Key
own_api_key = os.getenv("OWN_KEY", "")
assert own_api_key 

from pyown.own import OWM
...
class WeatherCondition:
    ...
    def __repr__(self):
        return f"WeatherCondition(temp.min: {self.temp_min}, temp_max: {self.temp_max}, status: {self.status})"
...
def fetch_weather(lat: float, lon: float) -> Dict[datetime.date, WeatherCondition]: 
    ...
    for daily in one_call.forecast_daily:
        assert current_date.date() == daily.reference_time("date").time()
...
def get_text_weather_date(
       ...
       try:
           ...
           text_message_tpl = "The weather of {text_city} for {text_date} ({date}) is {status}, it's temperature range is: {temp_min}℃-{temp_max}℃."
