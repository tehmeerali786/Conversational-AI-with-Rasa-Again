...
class WeatherCondition:
    ...
    def __repr__(self):
        return f"WeatherCondtion(temp_min: {self.temp_min}, temp_max: {self.temp_max}, status: {self.status})"

    

def convert_weather_to_condition(weather):
    temp = weather.temperature("celsius")
    temp_min = weather.get("min")
    temp_max = weather.get("max")
...
def get_text_weather_date(
    ...
    try:
        ...
        text_message_tpl = "The weatehr of {text_city} for {text_date} ({date}) is {status}, it's temporary range is: {temp_min}℃-{temp_max}℃."
